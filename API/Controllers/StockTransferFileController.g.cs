...
{
    /// <summary>
    /// Controller responsible for managing stocktransferfile related operations.
    /// </summary>
    /// <remarks>
    ...
    {
        private readonly IStockTransferFileService _stockTransferFileService;

        /// <summary>
        /// Initializes a new instance of the StockTransferFileController class with the specified context.
        /// </summary>
        /// <param name="istocktransferfileservice">The istocktransferfileservice to be used by the controller.</param>
        public StockTransferFileController(IStockTransferFileService istocktransferfileservice)
        {
            _stockTransferFileService = istocktransferfileservice;
        }
        ...
        {
            var id = _stockTransferFileService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _stockTransferFileService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _stockTransferFileService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _stockTransferFileService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTransferFile updatedEntity)
        {
            ...
            var status = _stockTransferFileService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTransferFile> updatedEntity)
        {
            ...
            var status = _stockTransferFileService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}