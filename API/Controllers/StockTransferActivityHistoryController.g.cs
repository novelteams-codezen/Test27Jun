...
{
    /// <summary>
    /// Controller responsible for managing stocktransferactivityhistory related operations.
    /// </summary>
    /// <remarks>
    ...
    {
        private readonly IStockTransferActivityHistoryService _stockTransferActivityHistoryService;

        /// <summary>
        /// Initializes a new instance of the StockTransferActivityHistoryController class with the specified context.
        /// </summary>
        /// <param name="istocktransferactivityhistoryservice">The istocktransferactivityhistoryservice to be used by the controller.</param>
        public StockTransferActivityHistoryController(IStockTransferActivityHistoryService istocktransferactivityhistoryservice)
        {
            _stockTransferActivityHistoryService = istocktransferactivityhistoryservice;
        }
        ...
        {
            var id = _stockTransferActivityHistoryService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _stockTransferActivityHistoryService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _stockTransferActivityHistoryService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _stockTransferActivityHistoryService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockTransferActivityHistory updatedEntity)
        {
            ...
            var status = _stockTransferActivityHistoryService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockTransferActivityHistory> updatedEntity)
        {
            ...
            var status = _stockTransferActivityHistoryService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}