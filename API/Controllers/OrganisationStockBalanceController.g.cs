...
{
    /// <summary>
    /// Controller responsible for managing organisationstockbalance related operations.
    /// </summary>
    /// <remarks>
    ...
    {
        private readonly IOrganisationStockBalanceService _organisationStockBalanceService;

        /// <summary>
        /// Initializes a new instance of the OrganisationStockBalanceController class with the specified context.
        /// </summary>
        /// <param name="iorganisationstockbalanceservice">The iorganisationstockbalanceservice to be used by the controller.</param>
        public OrganisationStockBalanceController(IOrganisationStockBalanceService iorganisationstockbalanceservice)
        {
            _organisationStockBalanceService = iorganisationstockbalanceservice;
        }
        ...
        {
            var id = _organisationStockBalanceService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _organisationStockBalanceService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _organisationStockBalanceService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _organisationStockBalanceService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] OrganisationStockBalance updatedEntity)
        {
            ...
            var status = _organisationStockBalanceService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<OrganisationStockBalance> updatedEntity)
        {
            ...
            var status = _organisationStockBalanceService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}