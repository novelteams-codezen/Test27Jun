...
{
    /// <summary>
    /// Controller responsible for managing purchaseorderworkflowstep related operations.
    /// </summary>
    /// <remarks>
    ...
    {
        private readonly IPurchaseOrderWorkFlowStepService _purchaseOrderWorkFlowStepService;

        /// <summary>
        /// Initializes a new instance of the PurchaseOrderWorkFlowStepController class with the specified context.
        /// </summary>
        /// <param name="ipurchaseorderworkflowstepservice">The ipurchaseorderworkflowstepservice to be used by the controller.</param>
        public PurchaseOrderWorkFlowStepController(IPurchaseOrderWorkFlowStepService ipurchaseorderworkflowstepservice)
        {
            _purchaseOrderWorkFlowStepService = ipurchaseorderworkflowstepservice;
        }
        ...
        {
            var id = _purchaseOrderWorkFlowStepService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _purchaseOrderWorkFlowStepService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _purchaseOrderWorkFlowStepService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _purchaseOrderWorkFlowStepService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PurchaseOrderWorkFlowStep updatedEntity)
        {
            ...
            var status = _purchaseOrderWorkFlowStepService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PurchaseOrderWorkFlowStep> updatedEntity)
        {
            ...
            var status = _purchaseOrderWorkFlowStepService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}