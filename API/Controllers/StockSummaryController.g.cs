...
{
    /// <summary>
    /// Controller responsible for managing stocksummary related operations.
    /// </summary>
    /// <remarks>
    ...
    public class StockSummaryController : ControllerBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the StockSummaryController class with the specified context.
        /// </summary>
        /// <param name="istocksummaryservice">The istocksummaryservice to be used by the controller.</param>
        public StockSummaryController(IStockSummaryService istocksummaryservice)
        {
            _stockSummaryService = istocksummaryservice;
        }
        ...
        {
            var id = _stockSummaryService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _stockSummaryService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _stockSummaryService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _stockSummaryService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] StockSummary updatedEntity)
        {
            ...
            var status = _stockSummaryService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<StockSummary> updatedEntity)
        {
            ...
            var status = _stockSummaryService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}