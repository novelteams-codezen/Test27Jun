...
namespace Test27Jun.Controllers
{
    ...
    public class GoodsReceiptActivityHistoryController : ControllerBase
    {
        ...
        {
            var id = _goodsReceiptActivityHistoryService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _goodsReceiptActivityHistoryService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _goodsReceiptActivityHistoryService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _goodsReceiptActivityHistoryService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] GoodsReceiptActivityHistory updatedEntity)
        {
            ...
            var status = _goodsReceiptActivityHistoryService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<GoodsReceiptActivityHistory> updatedEntity)
        {
            ...
            var status = _goodsReceiptActivityHistoryService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}